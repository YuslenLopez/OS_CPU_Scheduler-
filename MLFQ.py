...
-need 3 queues:
    -queue1 RR with tq=5
    -queue2 RR with tq=10
    -queue3 FCFS
-once proccess is downgraded it will not be upgraded 
    
-all proccesses enter the queue1 if cpu burst is bigger than tq=5 move to queue2 
-proccesses enter the queue2 if cpu burst is bigger than tq=10 move to queue3
- 
...
class Proccess:
    ...
    Arrival_Time=0
    #current CPU Burst (For keepiing track of how much CPU burst has gone by in Queue 1 and 2)
    Curr_CPU_Burst=0
...
#creating two more queues for MLFQ
queue2=[]
queue3=[] 

#creating queue copy to store for printing later 
queue4=[]
queue4.append(process1)
queue4.append(process2)
queue4.append(process3)
queue4.append(process4)
queue4.append(process5)
queue4.append(process6)
queue4.append(process7)
queue4.append(process8)

#creating waiting list for proccesses in IO
WaitingList=[]
#time quantums for queue 1 and 2 
Tq1=5
Tq2=10
...
while queue!=[] or WaitingList!=[]:
    if queue !=[]:
        #getting proccesses as they appear
        item=queue.pop(0)

        #Setting burst tracking variable to current cpu burst
        item.Curr_CPU_Burst=item.Burst[item.Burst_index]

        #if current CPU Burst greater than time quantum, subtract from burst, add to time and move proccess to next queue
        if(item.Curr_CPU_Burst>Tq1):

            #when proccess runs first time store initial starting time 
            if item.Burst_index==0:
                item.ResponseTime=t #this works cause all proccesses are arriving at t=0 and will run a little in first queue

            print("\n--------------------Proccess #",item.Proccess_Number)
            print("Time Quantum =", Tq1,"Exceeded. Moving to next queue")
            print("CPU burst =",item.Curr_CPU_Burst)
            item.Curr_CPU_Burst-=Tq1
            print("Left over CPU burst =",item.Curr_CPU_Burst) 
            print("Current time =",t)
            t+=Tq1
            item.Total_CPU_Burst+=Tq1
            ...
            #updating time by burst
            t=t+item.Curr_CPU_Burst

            #updating individual Proccess burst tracker 
            item.Total_CPU_Burst+=item.Curr_CPU_Burst

            #updating combined CPU burst of all process (for CPU utilization)
            Total_All_CPU_Burst+=item.Curr_CPU_Burst

            #set Current_CPU_Burst to zero
            item.Curr_CPU_Burst=0
            ...
            #print IO burst if not out of index
            if item.Burst_index+1<len(item.Burst)-1:
            ...
            if item.Burst_index+1<len(item.Burst)-1:
                ...
                print("Next Arrival Time =",item.Arrival_Time)#test for arrival times
            ...
            if item.Burst_index <= (len(item.Burst)-1):
                ...
                print("proccess completed")#test
                item.Completion_Time=t
        


    #index for waiting list
    k=0
    ...
    #if ready queue is empty and waiting is not we must increment the time till next proccess arrival
    if(queue==[]and WaitingList!=[]):
        print("Queue is empty waiting on process +1")
        t+=1

    #First queue loop end  
...
print("====================Second queue======================")
#Second queue loop 
while queue2!=[] or WaitingList!=[]:
    if queue2 !=[]:
        #getting proccesses as they appear
        item=queue2.pop(0)

        #if no burst left over set next burst
        if(item.Curr_CPU_Burst==0):

            #Setting burst tracking variable to current cpu burst
            item.Curr_CPU_Burst=item.Burst[item.Burst_index]

        #if current CPU Burst greater than time quantum, subtract from burst, add to time and move proccess to next queue
        if(item.Curr_CPU_Burst>Tq2):
            print("\n--------------------Proccess #",item.Proccess_Number) 
            print("Time Quantum =", Tq1,"Exceeded. Moving to next queue")
            print("CPU burst =",item.Curr_CPU_Burst)
            item.Curr_CPU_Burst-=Tq2
            print("Left over CPU burst =",item.Curr_CPU_Burst)
            print("Current time =",t)
            t+=Tq2
            item.Total_CPU_Burst+=Tq2
            print("Arrival time =",item.Arrival_Time)
            item.Arrival_Time=t
            print("Next Arrival time =",item.Arrival_Time)
            print("")
            ...
            #if proccess is not complete 
                #if arrival time is greater than current time add to waiting list 
                #else add back to the end of queue2
            #else proccess is finished and set completion time to current time (t)  
            if item.Burst_index <= (len(item.Burst)-1):
                if(item.Arrival_Time>t):
                    WaitingList.append(item)       
    ...
    #loop checking if any proccess in waiting list is ready to move to ready queue2 
    for n in WaitingList:
        #if arrival time of proccess is less than or equal to current time then add back to queue2 and remove from waiting list 
        if n.Arrival_Time<=t:
            queue2.append(n)
            WaitingList.pop(k)
        k+=1
    
    #if ready queue2 is empty we must increment the time till next arrival
    if(queue2==[] and WaitingList!=[]):
        print("Queue is empty waiting on process +1")
        t+=1

    #First queue2 loop end  
...
"""
-kept older code for FCFS, might need to adjust sightly for current cpu_burst maybe smaller burst than in list 
- 
...
#Loop to execute until queue3 is empty 
while queue3 !=[]:

    #sorting the queue3 by the arrival time in class to order by FCFS
    queue3 = sorted(queue3, key=p_sort)
    
    #getting first proccess in queue3 
    item=queue3.pop(0)

    #if some burst left over we dont set new burst
    if(item.Curr_CPU_Burst==0):
    ...
    #if proccess is not complete add back to end of queue3, else proccess is finished and set completion time to current time (t)  
    if item.Burst_index <= (len(item.Burst)-1):
        queue3.append(item)
    else:
        print("Proccess Completed")
